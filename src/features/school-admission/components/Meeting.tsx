...
import { RequirementsAccordion } from "../requirements/compenents/Requirements";
...
const Meeting = ({ status }: { status: any }) => {
  ...
  return status?.meeting_status == "2" ? (
    <>
      ...
      <section className="card col p-3 gap-3">
        <div className="">
          ...
          <p>
            ...
            <b>{dayjs(status.message.date).format("dddd, MMMM D, YYYY")} </b> at{" "}
            <b>{status.message.time}</b>.
          </p>
          <a
        ...
        <Modal open={open} setOpen={toggleModal} title="Reschedule Meeting">
          <div className="p-3 w-[80vw] md:w-[60vw] xl:w-[45vw] col gap-2">
            <p>
              ...
              the current one. Failing to attend a meeting and not rescheduling
              in advance will lead to you <strong>NOT</strong> being able to
              book another career advisory meeting for <strong>1 day</strong>.
            </p>
    ...
    <section className="px-4">
      ...
      <div className="card p-4 col gap-1">
        <p className="">
          Please wait as our team updates the proposed schools for your viewing. This process will be
          completed within the next 24 hours.
        </p>
        <p className="">Thank you for your patience.</p>
        <p className="">
          For any inquiries, please <strong>create a ticket</strong> to contact
          support.
        </p>
        <Link
          to="/create-ticket"
          className="primary-border-btn p-0 my-2 self-end"
        >
          Create Ticket
        </Link>
      </div>
