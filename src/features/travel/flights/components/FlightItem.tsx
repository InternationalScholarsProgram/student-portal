import dayjs from "dayjs";
import { convertDuration } from "./utils";

function FlightItem({ offer }: { offer: any }) {
  const offerSlice = offer?.slices[0];
  const noOfFlights = offerSlice?.segments?.length || 0;

  const firstSegment = offerSlice.segments[0];
  const lastSegment = offerSlice.segments.at(-1);

  const departureTime = dayjs(firstSegment.departing_at).format("hh:mm A");
  const arrivalTime = dayjs(lastSegment.arriving_at).format("hh:mm A");

  const duration = convertDuration(offerSlice.duration);
  return (
    <div className="flight-item-container card shadow">
      <div className="card-header d-flex flex-row align-items-center gap-2">
        <img
          alt="Carrier Logo"
          className="img-fluid max-h-[30px]"
          src={offer?.owner?.logo_symbol_url}
        />
        <p className="m-0">{offer?.owner?.name}</p>
      </div>

      <div className="flight-item">
        <div className="flight-item-details">
          <div className="details-card">
            <h3>{departureTime}</h3>
            <span>{firstSegment.origin.iata_code}</span>
          </div>
          <div className="flex-grow-1 d-flex flex-column mx-2">
            <div className="line-container">
              <div className="line"></div>
              <div className="div-svg">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="16"
                  height="16"
                  fill="currentColor"
                  className="bi bi-airplane"
                  viewBox="0 0 16 16"
                >
                  <path d="M6.428 1.151C6.708.591 7.213 0 8 0s1.292.592 1.572 1.151C9.861 1.73 10 2.431 10 3v3.691l5.17 2.585a1.5 1.5 0 0 1 .83 1.342V12a.5.5 0 0 1-.582.493l-5.507-.918-.375 2.253 1.318 1.318A.5.5 0 0 1 10.5 16h-5a.5.5 0 0 1-.354-.854l1.319-1.318-.376-2.253-5.507.918A.5.5 0 0 1 0 12v-1.382a1.5 1.5 0 0 1 .83-1.342L6 6.691V3c0-.568.14-1.271.428-1.849m.894.448C7.111 2.02 7 2.569 7 3v4a.5.5 0 0 1-.276.447l-5.448 2.724a.5.5 0 0 0-.276.447v.792l5.418-.903a.5.5 0 0 1 .575.41l.5 3a.5.5 0 0 1-.14.437L6.708 15h2.586l-.647-.646a.5.5 0 0 1-.14-.436l.5-3a.5.5 0 0 1 .576-.411L15 11.41v-.792a.5.5 0 0 0-.276-.447L9.276 7.447A.5.5 0 0 1 9 7V3c0-.432-.11-.979-.322-1.401C8.458 1.159 8.213 1 8 1s-.458.158-.678.599" />
                </svg>
              </div>
              <div className="line"></div>
            </div>
            <div className="col items-center w-full">
              <span>{duration} </span>
              <span>
                {noOfFlights === 1 ? "Direct" : noOfFlights + " stops"}
              </span>
            </div>
          </div>
          <div className="details-card">
            <h3>{arrivalTime}</h3>
            <span>{lastSegment.destination.iata_code}</span>
          </div>
        </div>
        <div className="flight-item-price">
          <p>{offer?.total_amount}</p>
          <button className="primary-border-btn">Select</button>
        </div>
      </div>
    </div>
  );
}

export default FlightItem;
