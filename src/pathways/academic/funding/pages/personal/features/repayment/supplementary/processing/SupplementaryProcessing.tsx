...
import SupplementaryDecision from "./SupplementaryDecision";
import ApplicationForm from "../../../../../../components/ApplicationForm";

/** Shared status card (same visual style as Personal) */
...
const SupplementaryProcessing = () => {
  /**
   * Mirror the Personal flow using supplementary_loan.status
   * 1 → Decision (accept/decline)
   * 2 → Accepted → Contract
   * 3 → Declined — Under Review
   * 4 → Provide bank details
   * 5 → Admin recall — In Review
   * 6 → Requirements Adjusted → accept again
   * 7 → No Adjustments — accept again
   * 8 → Totally rejected → show reason + re-apply form
   * 9 → Contract rejected
   */
  const { supplementaryLoan, user_details: application, invalidate } = usePersonal();
  const s = Number(supplementaryLoan?.status ?? 0);

  const maxForReapply = Number(supplementaryLoan?.principal) || 0;

  switch (s) {
    case 1:
      // Decision pending
      return <SupplementaryDecision />;

    case 2:
      // Accepted -> show supplementary contract
      return (
        <AcceptedLoan>
    ...
    case 3:
      ...
      return (
        <StatusCard
          variant="error"
          title="Supplementary Loan Declined — Under Review"
    ...
    case 4:
      ...
      return (
        <BankDetails
          loan={{
            member_no: supplementaryLoan?.member_no || "",
            fullnames: supplementaryLoan?.fullnames || "",
            phone: supplementaryLoan?.phone || "",
            loan_id: supplementaryLoan?.loan_id || "",
    ...
    case 8:
      // Totally rejected — show reason & re-apply form (supplementary)
      return (
        <>
          <StatusCard
            variant="error"
            ...
            content={
              <div className="col gap-2">
                <p>
                  Your supplementary loan application has been
                ...
                {supplementaryLoan?.to_pay != null && (
                  <p className="px-3">
                    <b>Amount to pay:</b>{" "}
                    <span className="font-semibold">
                      {formatCurrency(Number(supplementaryLoan?.to_pay))}
                    </span>
                  </p>
                )}
    ...
    default:
      return (
        <StatusCard
          variant="info"
          title="Supplementary Loan — Status Update"
...
export default SupplementaryProcessing;
