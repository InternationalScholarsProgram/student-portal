    ...
    <ContentComponent header="Repayment Schedule">
      {upcoming?.length > 0 && (
        <p>
          ...
          <strong>{formatCurrency(Number((upcoming[0] as any)?.scheduled_payment || 0))}</strong>{" "}
          is due on{" "}
          <strong>
            {formatDate(
...
const upcomingColumns: GridColDef[] = [
  ...
  { field: "scheduled_payment", flex: 1, headerName: "To Pay", type: "number",
    renderCell: (p) => formatCurrency(Number(p?.row?.scheduled_payment ?? 0)) },
  { field: "interest_rate", type: "number", flex: 1, headerName: "Interest",
    renderCell: (p) => String(Number(p?.row?.interest_rate ?? 0)) },
  { field: "new_balance", flex: 1, headerName: "New Balance", type: "number",
    renderCell: (p) => formatCurrency(Number(p?.row?.new_balance ?? 0)) },
  { field: "status", headerName: "Status", flex: 1,
    renderCell: (p) => (
      <div className="row-center flex-1 h-full w-full">
        <p className="w-fit"><StatusChip type={(p?.row?.status as string) || "Pending"} /></p>
      </div>
    ),
  },
];
...
const historyColumns: GridColDef[] = [
  ...
  { field: "status", headerName: "Status", flex: 1,
    renderCell: (p) => (
      <div className="row-center flex-1 h-full w-full">
        <p className="w-fit"><StatusChip type={(p?.row?.status as string) || "Paid"} /></p>
      </div>
    ),
  },
];
