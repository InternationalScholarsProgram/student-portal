...
const SupplementaryContract = () => {
  ...
  const {
    user_details: application,
    supplementaryLoan: loan,
    invalidate,
  } = usePersonal();

  // Repayment schedule (reuse personalEndpoints for now)
  const {
    data: schedulePayments,
    isLoading,
    error,
  } = useQuery({
    queryKey: [user?.email, "repayment-schedule", "supplementary", loan?.loan_id],
    queryFn: () => personalEndpoints.repaymentSchedule(loan),
    select: (response) => {
  ...
  const makePdfFile = async (): Promise<File> => {
    ...
    const name = `${loan?.loan_id}_${loan?.fullnames}_supplementary_contract.pdf`;
  ...
  const handleSign = async () => {
    try {
      ...
      // Reuse personal endpoint for signing
  ...
  const handleReject = async () => {
    try {
      ...
      // Reuse personal endpoint for reject
      await personalEndpoints.signContract(payload);
  ...
  return (
    <>
      ...
      <Modal title="Supplementary Loan Contract" open={openContract} setOpen={toggleContractModal}>
        <div className="relative w-[95vw] md:w-[80vw] xl:w-[70vw] h-[80vh] p-2 overflow-y-auto">
          <div ref={targetRef} className="col w-full">
            ...
            <article className="py-3 col gap-2">
              <b className="underline uppercase">RE: Supplementary Loan Contract</b>
              <section className="col gap-3">
                <b>
                  This supplementary loan agreement (this "agreement") dated {formatDate(new Date())}.
                </b>
                <span>BETWEEN</span>
                <p>
                ...
                <div className="col gap-3 p-3">
                  <div className="col gap-2">
                    ...
                    <p>
                      ...
                      supplementary needs and the borrower promises to repay this principal amount to the lender, paying
                      on the unpaid principal amount at the rate of <b>12%</b> per annum, beginning on{" "}
